                ...
                <div class=" container-fluid ">
                  ...
                  <div class="collapse navbar-collapse" id="mynavbar">
                    ...
                    <form class="d-flex  col-11 ">
                      <input class="form-control rounded-pill  me-2 h-25  " v-model="searchVal" @keyup="searchFilter"
                        @click="search = true" type="text" placeholder="Search">
                      <button class="btn btn-primary  rounded-pill m-0" type="button">Search</button>
                    </form>




                    <ul class=" dropdown-menu mt-4  searchCont  list-group list-group-flush border-0"
                      style="margin-top:10px" v-show="search" aria-labelledby="dropdownMenuButton">

                      <li class="list-group-item ml-8 border-0" v-for="pub in result" :key="pub.id"
                        @click="filterData(pub.id)">{{
                            pub
                              .title
                        }}</li>
            ...
            <!-- <nav class="  navbar-expand-sm">
              <nav class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="mynavb">
                  <div class="row d-flex mt-2 mb-5">
              ...
              <div class="col-lg-3 col-md-3 col-xl-3 col-sm-1">
                <div class="card  mb-10">
                  ...
                  <div class="card-body text-dark " v-if="!plus">
                    <h6 class="text-dark   "> Titre des Publicites</h6>
                    <div class="text-sm">
                  ...
                  <div class="mb-2">
                    <ul class=" list-group list-group-flush border-0" v-for="pub in listPub" :key="pub.id">
                      <li class="list-group-item ml-5  text-sm border-0" :id="pub.id" @click="filterData(pub.id)">{{
                          pub
                            .title
                      }}</li>
                    </ul>
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      await getListPub();
      filterData(VueCookies.get('idPublicite'))
    ...
    function filterData(id) {
      ...
      let selectedCAt = document.getElementById(id);
    ...
    function searchFilter() {
      ...
      for (var i = 0; i < listPub.value.length; i++) {
        if (listPub.value[i].title.toUpperCase().includes(searchVal.value.toUpperCase())/*  || listPub.value[i].description.toUpperCase().includes(searchVal.value.toUpperCase()) */) {
          result.value.push(listPub.value[i]);
        }
      }
      // for (var j = 0; j < listPubSave.value.length; j++) {
      // for (var k = 0; k < result.value.length; k++) {
      //   if (listPubSave.value[j].title.toUpperCase()==result.value[k].title.toUpperCase()) {

      //     result.value.push(listPubSave.value[j]);
      //   }
      //   }
      // }
      if (result.value.length != 0) {  //  
        ...
        search.value = false
        listPub.value = listPubSave.value
    ...
    async function getListPub() {
      ...
      if (response.status) {
        ...
        listPubSave.value = listPub.value
        // eslint-disable-next-line require-atomic-updates
        loading.value = !loading.value
      }
      else {
